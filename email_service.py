...
def _html_link_nova_senha(nome: str, link: str) -> str:
    """Corpo HTML do e-mail com o link para criar uma nova senha (visual AGRIVIA)."""
    saudacao = f"Olá, {nome}!" if nome else "Olá!"
    return f"""<!doctype html>
<html lang="pt-br">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"></head>
        ...
        <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="max-width:520px; width:100%; background:#ffffff; border-radius:14px; overflow:hidden; box-shadow:0 6px 18px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:32px;">
              <h1 style="color:#2b3a22; font-size:20px; margin:0 0 16px 0;">{saudacao}</h1>
              <p style="color:#3a4a30; font-size:15px; line-height:1.6; margin:0 0 18px 0;">
                Recebemos um pedido para criar uma <b>nova senha</b> de acesso ao <b>AGRIVIA</b>.
                Clique no botão abaixo para definir a sua nova senha:
              </p>
              <table role="presentation" cellpadding="0" cellspacing="0" style="margin:24px 0;">
                <tr>
                  <td align="center" style="background:#476126; border-radius:8px;">
              ...
              <p style="color:#98a890; font-size:12px; line-height:1.6; margin:24px 0 0 0;">
                ...
                Se você não pediu a troca de senha, pode ignorar este e-mail — nada será alterado.
              </p>
            </td>
          </tr>
          <!-- Rodapé -->
...
def enviar_link_nova_senha(destino: str, nome: str, link: str) -> bool:
    """Envia o e-mail com o link de nova senha. Retorna True se aceito pelo Resend."""
    html = _html_link_nova_senha(nome, link)
    return enviar_email(destino, "Crie sua nova senha - AGRIVIA", html)
